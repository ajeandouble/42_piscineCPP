...
{
	std::cerr << "ClapTrap constructor has been called" << std::endl;
	this->_name = name;
...
ClapTrap::~ClapTrap()
{
	std::cerr << "ClapTrap destructor has been called" << std::endl;	
}
...
{
	this->_hit_points = this->_random(0, this->_max_hit_points);
	std::cout << "CL4P-TP " << this->_name << " attacks " << target << " melee-style, causing ";
...
int ClapTrap::_random(int min, int max)
{
